...
## 1. 新增專案成員 MCP 工具
- [x] 1.1 在 `mcp_server.py` 新增 `add_project_member` 函數
...
## 2. 新增專案里程碑 MCP 工具
- [x] 2.1 在 `mcp_server.py` 新增 `add_project_milestone` 函數
  - 呼叫 `await ensure_db_connection()` 確保資料庫連線
...
## 3. 更新 Agent Prompts
- [x] 3.1 更新 `linebot_agents.py` 中的群組對話 prompt
  - 加入 `add_project_member` 工具說明
  - 加入 `add_project_milestone` 工具說明
- [x] 3.2 更新 `013_update_linebot_prompts.py` migration 檔案
  - 同步新的 prompt 內容
- [x] 3.3 直接更新資料庫中的 prompt 資料
