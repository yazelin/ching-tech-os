...
- [x] 檢測 URL 中的 shareToken 參數
- [x] 顯示密碼輸入對話框
- [x] 呼叫後端 API 驗證密碼
- [x] 驗證成功後載入內容
- [x] 錯誤處理（密碼錯誤、連結過期、已鎖定）
...
> 測試過程中發現並修復的問題：
> - 驗證器 regex bug：`\s*` 匹配換行符 → 改為 `[ \t]*`（:: right :: 和 ::: 結束標記）
> - 前端 API 響應解析：`data.content` → `data.data.content`
> - 密碼長度：後端 6 位 → 4 位（與前端一致）
> - MD2PPT/MD2DOC 附件：新增保存檔案到 NAS 功能

### 10. 清理過期分享 ✅
- [x] 建立定期清理任務或在 API 中清理過期記錄
- [x] 清理超過 24 小時的分享連結

**驗證**：過期連結會被自動清理 ✅

> 注意：清理任務已在 `scheduler.py` 中實作，每小時執行 `cleanup_expired_links()`
