...
**Phase 6.5 checkpoint（2026-08-17）**：以開發機（同時為 staging，服務直跑工作目錄）完成演練：(1) 服務重啟——11:56 重啟後 health check 通過、新路由（providers/status）生效、extends/MCP 正常載入，舊 process 停止碼造成的一次性 systemd "Failed" 為停止行為非啟動失敗；(2) 重啟後完整 preflight（含真實 handshake）8/8 通過；(3) adapter 缺失（`CODEX_ACP_BIN_PATH=/nonexistent`）→ preflight fail-closed、version 檢查略過；(4) 登入過期／auth 不存在（`CODEX_HOME` 指向空目錄）→ `auth_storage` fail、handshake 不啟動 subprocess；(5) forced Claude rollback 目標狀態即目前狀態（`AI_PROVIDER_MODE` 未設定 → 預設 claude），mode 切換演練（codex/auto → claude）留待啟用 canary 時與 9.6 一併執行。
...
**Phase 8.5 checkpoint（2026-08-18）**：Go 之後經營運確認（最終目標為 Claude/Codex 完整雙系統備援），一般對話分兩步啟用：第一步先將 `linebot-group` 加入 canary contexts（群組多為查詢型使用，唯讀工具影響最小）；`linebot-personal`、`telegram-*` 與 restricted 留待實際切換表現確認後再加。已於 16:56 重啟生效（0.10.0、9.6 例外修正與 usage 退避一併上線），重啟後 health/openapi 版本/usage monitor 驗證通過。取捨已明確記錄：>=90% 窗口內群組用戶為「可查詢、不可執行副作用」的 Codex 服務，優於 Claude 限流時的無服務。

**Phase 9.7/9.8 Go/No-Go（2026-08-18）**：canary 於 2026-08-17 12:24 啟用（auto mode，contexts=internal_admin/internal_test/test + test-agent），連續觀察 25 小時。證據：(1) 全部 9 筆有路由記錄的請求 provider/route_reason 皆可辨識且正確（含 9.6 演練的 forced_codex×4、codex_unready、forced_claude 與真實流量的 canary_not_allowed）；(2) 不可追查請求 0、安全事件 0、重複副作用 0、secrets 洩漏 0；(3) usage monitor 除 8/17 17:12–20:50 的 token 過期盲區（fail-safe 留 Claude、自動恢復，已據此新增退避與 nudge 改進）與兩次 ISP 換 IP 瞬斷外全程 fresh。**結論：Go**——現行 canary scope（internal/test）維持 auto mode；擴大至一般對話（8.5）另行評估，需權衡「>=90% 時一般用戶暫時只有唯讀工具的 Codex」vs「Claude 限流無服務」的產品取捨。觀察樣本中真實服務端流量僅 1 筆（低流量時段），路徑正確性主要由 9.6 演練矩陣背書。版本 bump 0.9.3 → 0.10.0（10.4）。
