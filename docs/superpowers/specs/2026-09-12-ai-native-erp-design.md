...
5. **不做**：BOM、多公司、多幣別。
   ...
   > **2026-09-15 修訂**：原本這條還包含「會計分錄、發票、稅務、報價／銷售單據」，
   > 已由 yazelin 拍板移除。當時的判斷基於 ERPNext 只有 1 張銷貨單、0 筆分錄；
   > 舊 ERP 傾印 `CH001_export` 有 17,649 張銷貨單與 319,121 筆分錄（借貸 100% 平衡），
   > 前提變了。會計、發票、應收應付、票據的重建見
   > `2026-09-15-ch001-erp-rebuild-design.md`。稅務（申報）仍然不做。
