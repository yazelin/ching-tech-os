...
日期：2026-09-12
狀態：**已被推翻，不要照這份實作**（2026-09-18）

> **2026-09-18 推翻**：改以自製的 ct_erp（`http://ct.erp`，實際位址 192.168.11.6:8000）為 ERP 的唯一事實來源，
> 本規格描述的 CTOS 自建模組（`parties`／`items`／`purchase_orders` 三組表與對應 MCP 工具）退場。
>
> 理由：ct_erp 已經是會計在用的活系統，裝著十一萬多筆傳票、四萬多筆進貨單與實際銀行帳號，
> 功能範圍（含傳票、收付款、開戶帳號）是本規格刻意不做的那一塊的超集；而本規格的模組雖然已部署到
> 192.168.11.11（alembic 033），裡面是空的（`/api/items`、`/api/purchase-orders` 都回 `total: 0`，
> 匯入腳本 #197 從未執行），退場的代價接近零。
>
> 現行方向見 kb-233（ct_erp 實測狀態）、kb-232（ct_erp OpenAPI 規格）、kb-230（域名與 IP 對照）。
> 域名已於 2026-09-18 分家：`ct.erp` 指 ct_erp，ERPNext 改用 `next.erp`。
>
> 原狀態：已拍板（設計原則由管理層定：AI Agent 能操作為第一優先），實作中；PR 1 後端 #192 已合併
