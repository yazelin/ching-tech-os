...
- **專案模組重建（後端）**：ERPNext 停用後專案進度回到自己的系統。四張表 `projects` /
  ...
  里程碑未完成、且所屬專案還在進行中。讀取只要有 `project-management` 權限，建立與刪除限管理員，
  編輯限管理員或該專案成員。畫面在新前端 ctos-web，進行中。
...
- **Agent 切換指令**：`/agent` 指令讓管理員在群組或個人對話中切換 AI Agent，每個群組/個人獨立偏好，支援名稱和編號切換。`/agent restricted` 可設定群組中未綁定用戶使用的受限模式 Agent。
