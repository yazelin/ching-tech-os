...
| 桌面介面 | AI Log |
|:---:|:---:|
| ![桌面](demo/02-桌面概覽.png) | ![AI Log](demo/15-AI-Log.png) |

| Line Bot 管理 | 知識庫 |
|:---:|:---:|
| ![Line Bot](demo/11-LineBot-群組.png) | ![知識庫](demo/14-知識庫.png) |
...
| Line Bot | 完成 | 群組管理、訊息記錄、用戶綁定、AI 對話整合、MCP 工具、NAS 檔案搜尋與發送、AI 圖片生成（含 Hugging Face FLUX 備用）、文件讀取、群組 @ 提及回覆、回覆引用、自訂記憶、物料/庫存管理、簡報生成、MD2PPT/MD2DOC 文件轉換、統一斜線指令（/start、/help、/reset、/debug）、歡迎訊息（加好友時自動推送） |
...
- **Bot 多模式平台**：未綁定用戶可選擇 `restricted` 模式（受限 AI 對話）或 `reject` 模式（拒絕並提示綁定），透過 `BOT_UNBOUND_USER_POLICY` 環境變數控制。受限模式支援頻率限制（`BOT_RATE_LIMIT_HOURLY` / `BOT_RATE_LIMIT_DAILY`）。
- **統一斜線指令**：LINE 和 Telegram 共用 CommandRouter 框架，統一 `/start`、`/help`、`/reset`、`/debug` 指令。支援透過 `BOT_CMD_DISABLED` 環境變數停用特定指令。`/help` 動態列出可用指令（依平台、角色、啟用狀態過濾）。
- **管理員診斷指令**：`/debug` 指令讓管理員透過 AI Agent（bot-debug）分析系統日誌，快速定位問題。
- **LINE 歡迎訊息**：用戶加好友（FollowEvent）時自動推送歡迎訊息，引導帳號綁定。
- **背景研究功能**：research-skill 支援 Brave Search API，研究產出品質提升，並新增 stale timeout 防護。
- **多產業模組化**：`ENABLED_MODULES`（預設 `*`）控制後端路由、MCP 工具、排程任務與前端 App 顯示。
- **Skill contributes**：`SKILL.md` 支援 `contributes`（app / mcp_tools / scheduler）宣告，Hub 搜尋同時支援 ClawHub 與 SkillHub。
