...
async def process_message_with_ai(
    ...
    try:
        ...
        # 從 SkillManager 動態取得使用者可用工具
        try:
            from ..skills import get_skill_manager
            sm = get_skill_manager()
            skill_tools = sm.get_tool_names(app_permissions)
            # 合併 Agent 設定的內建工具 + SkillManager 的 skill 工具
            all_tools = list(dict.fromkeys(agent_tools + skill_tools))  # 去重保序
            logger.info(f"SkillManager 提供 {len(skill_tools)} 個工具，合計 {len(all_tools)}")
        except Exception as e:
            logger.warning(f"SkillManager 工具載入失敗，fallback 到硬編碼: {e}")
            # Fallback: 原本的硬編碼邏輯
