...
- generate_md2ppt: 產生專業簡報（MD2PPT 格式）
...
- generate_md2doc: 產生專業文件（MD2DOC 格式）
...
【文件/簡報使用情境】
1. 用戶說「幫我做一份簡報介紹公司產品」
   → generate_md2ppt(content="公司產品介紹簡報，包含...", ctos_tenant_id=...)
2. 用戶說「製作一份 5 頁的技術報告」
   → generate_md2ppt(content="技術報告...", slides_count=5, ctos_tenant_id=...)
3. 用戶說「幫我寫一份操作說明書」
   → generate_md2doc(content="操作說明書...", document_type="說明書", ctos_tenant_id=...)
4. 用戶說「做一份教學文件說明如何使用系統」
   → generate_md2doc(content="系統使用教學...", document_type="教學", ctos_tenant_id=...)
...
- generate_md2ppt: 產生簡報（content 必填，style/slides_count 可選，回傳 url 和 password）
- generate_md2doc: 產生文件（content 必填，document_type 可選，回傳 url 和 password）
