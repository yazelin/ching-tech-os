"""update linebot prompts with send_nas_file tool
...
加入 send_nas_file 工具說明，可直接發送圖片到 Line 對話中。
"""
...
- send_nas_file: 直接發送檔案給用戶（推薦使用）
  · file_path: 檔案完整路徑（從 search_nas_files 取得）
  · line_user_id / line_group_id: 從【對話識別】區塊取得
  · 圖片（jpg/png/gif 等）< 10MB 會直接顯示在對話中
  · 其他檔案會發送下載連結
...
10. 用戶要求找專案檔案時（如「找亦達 layout pdf」）：
    ...
    - 從結果列表中選擇最相關的檔案
    - 若找到多個檔案，列出選項讓用戶選擇
    - 用戶確認後，用 send_nas_file 直接發送（圖片會顯示、其他發連結）
    - 若只想給連結不發送，才用 create_share_link
...
- send_nas_file: 直接發送檔案（圖片會顯示、其他發連結）
- create_share_link: 只產生連結不發送
...
def upgrade() -> None:
    ...
    op.execute(f"""
        ...
        SET content = $prompt${LINEBOT_PERSONAL_PROMPT}$prompt$,
            description = 'Line Bot 個人對話使用，包含完整 MCP 工具說明（含 send_nas_file）',
            updated_at = NOW()
    ...
    op.execute(f"""
        ...
        SET content = $prompt${LINEBOT_GROUP_PROMPT}$prompt$,
            description = 'Line Bot 群組對話使用，精簡版包含 MCP 工具說明（含 send_nas_file）',
            updated_at = NOW()
