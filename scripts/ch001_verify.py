...
4. **借貸平衡**：`KJSNHB`（過帳後的分錄，借方貸方分兩欄）每張傳票借方＝貸方。
   這是會計完整性最強的指標，少一筆分錄就不可能平。

   `KJSNFB` 是**畫面輸入的明細**（欄 8 借貸別 ±1 × 欄 9 金額），只平 87.24%，
   **不要拿它當分錄來源**。原因已查明：收入（類別 1）與支出（類別 2）傳票，
   使用者只輸入對方科目，現金／銀行那一邊由系統自動補，只存在於 `KJSNHB`。
   實測收入 437 張、支出 14,738 張全部差 1 筆；轉帳（類別 3）103,771 張差 0 筆，
   而 103,771 正好等於 `KJSNFB` 能平的張數。所以那 12.76% 不是資料缺漏。
